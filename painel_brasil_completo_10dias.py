...
# ================== CAMINHOS (Render + Local) ==================
BASE_DIR = Path(__file__).resolve().parent

# Render: arquivos ficam no repositório (mesma pasta do .py) ou em subpasta
# Local: fallback pro seu caminho
...
ARQ_PREV = pick_path(
    ...
    BASE_DIR / "data" / "previsao_brasil_10dias.xlsx",
    PASTA_LOCAL / "previsao_brasil_10dias.xlsx",
)

ARQ_ATTR = pick_path(
    BASE_DIR / "arquivo_completo_brasil.xlsx",
    BASE_DIR / "data" / "arquivo_completo_brasil.xlsx",
    PASTA_LOCAL / "arquivo_completo_brasil.xlsx",
)

# ✅ agora é JSON (mas continua sendo GeoJSON no conteúdo)
GEOJSON_MUN = pick_path(
    BASE_DIR / "municipios_br.json",
    BASE_DIR / "data" / "municipios_br.json",
    PASTA_LOCAL / "municipios_br.json",
    PASTA_LOCAL / "Shapes_mun" / "municipios_br.geojson",
)
...
# ================== CONFIG (download gráfico em alta qualidade) ==================
# ✅ o ícone de câmera do Plotly vai baixar PNG com scale=3 (bem melhor)
GRAPH_CONFIG = {
    "scrollZoom": True,
    "displaylogo": False,
    "responsive": True,
    "toImageButtonOptions": {"format": "png", "filename": "grafico_temperaturas", "scale": 3},
                    ...
                    [
                        ...
                        html.Div(
                            ...
                            style={"display": "grid", "gridTemplateColumns": "repeat(4, 1fr)", "gap": "8px",
                                   "alignItems": "stretch", "marginTop": "2px"},
                        ),
                            ...
                            [
                                    ...
                                    [
                                        ...
                                        html.Div(
                                            ...
                                            style={
                                                ...
                                                "padding": "8px", "minHeight": "48px", "maxHeight": "24vh",
                                                "overflowY": "auto", "backgroundColor": "#fff",
                                            },
                                        ),
                                        html.Div(
                                            [
                                                html.Button("Exportar XLSX (≠ Normal)", id="btn-export-ehf", n_clicks=0),
                                ...
                                html.Div(
                                    [
                                        ...
                                        html.Div(
                                            [
                                                html.Button("Exportar XLSX (≠ Normal)", id="btn-export-risk", n_clicks=0,
                                                            disabled=(not HAS_RISK)),
                ...
                html.Div(
                    [
                        dcc.Graph(
                            ...
                            config=GRAPH_CONFIG,  # ✅ botão de download PNG HQ no modebar
                        ),
                        html.Div(
                            "Dica: use o ícone de câmera (no gráfico) para baixar o PNG em alta qualidade.",
...
def cb_viz(idx_date, reg_key, uf_keys, muni_key, layer, relayout):
    ...
    fig_bar = px.bar(
        bt, x="data_lbl", y="Valor", color="Série",
    ...
    fig_bar.update_xaxes(
        categoryorder="array",
        categoryarray=serie["data_lbl"].tolist(),
        showgrid=False,
        fixedrange=True,
    )
...
# =========================
# ✅ EXPORT XLSX (somente ≠ Normal)
# =========================
def _df_export_nao_normal(modo="ehf"):
    df = base.copy()
    ...
    # filtro leve (o que salva seu Render)
    if modo == "risk" and "risk_class" in df.columns:
        df = df[df["risk_class"].fillna("Normal") != "Normal"]
    else:
        df = df[df["classification"].fillna("Normal") != "Normal"]
    ...
    out = (df[keep]
           .rename(columns={k: cols[k] for k in keep})
           .sort_values(["Data", "UF", "NM_MUN"]))
    out["Data"] = pd.to_datetime(out["Data"]).dt.date
    return out

def _send_xlsx_bytes(out_df: pd.DataFrame, filename: str):
    def write_bytes(buf):
        bio = BytesIO()
        out_df.to_excel(bio, index=False)
...
def exportar_ehf_nao_normal(n_clicks):
    if not n_clicks:
        return no_update

    out = _df_export_nao_normal(modo="ehf")
    fname = "ehf_nao_normal.xlsx" if not DATES else f"ehf_nao_normal_{DATES[0].isoformat()}_a_{DATES[-1].isoformat()}.xlsx"
    return _send_xlsx_bytes(out, fname)
...
def exportar_risco_nao_normal(n_clicks):
    if not n_clicks:
        return no_update

    out = _df_export_nao_normal(modo="risk")
    fname = "risco_nao_normal.xlsx" if not DATES else f"risco_nao_normal_{DATES[0].isoformat()}_a_{DATES[-1].isoformat()}.xlsx"
    return _send_xlsx_bytes(out, fname)
...
if __name__ == "__main__":
    ...
    # Render usa PORT via env; local usa 8069
